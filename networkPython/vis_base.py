...
def nx2vis(G):
    ...
    # -----------------------------
    # Attribute mapping dictionaries
    # -----------------------------
    ...
    for u, v, data in G.edges(data=True):
        ...
        # Copy supported vis.js edge attributes directly
...
def visnet(G=None, nodes=None, edges=None, network_title='Atsaniik', description_df=None, description_title="Introduce your network", writeHTML="network_visualization.html", browserView=False, min_default_node_size=0, min_default_edge_width=0, maximum_display=100):
    """ visualize your network with a save button to export selected nodes and edges to JSON files
    Args:
        G (networkx.Graph | None): NetworkX graph (overrides nodes & edges)
    ...
    folder_path = Path(f"{Path(__file__).parent.resolve()}/ntOutPut/")
...
if __name__ == "__main__":
    ...
    visnet(nodes=nodes, edges=edges, network_title='small Network', browserView=True, writeHTML='test_small.html', min_default_node_size=1, min_default_edge_width=0, maximum_display=10)
